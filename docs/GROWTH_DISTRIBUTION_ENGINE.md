...
- Telegram / Discord: short announcements only in channels that explicitly allow them.
- Reddit: human-approved only; use official showcase/self-promotion threads and follow each community's current rules.
...
4. Build-in-public: a real problem, test, failure, or design decision.
5. Checklist/resource: useful without requiring signup.
...
- Do not use scraped personal emails for bulk unsolicited mail.
...
- Thu: cost/runtime guardrail note.
- Fri: community question + selected self-promo/showcase thread.
...
4. pilot-yes rate;
5. design-partner rate;
6. fraud/low-quality rate.

Stop spending time on channels that produce volume without qualified demand.
