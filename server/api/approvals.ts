...
export const approvalsRouter = router({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await dbClient.execute(
        `UPDATE pending_approvals
         SET status = 'approved', resolved_at = NOW(), resolved_by = ?, resolution_note = ?
         WHERE approval_id = ?`,
        [ctx.user.id, input.note ?? null, input.approvalId],
      );

      logger.info(
        { approvalId: input.approvalId, userId: ctx.user.id },
        "[Approvals] Approved",
      );
  ...
  reject: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await dbClient.execute(
        `UPDATE pending_approvals
         SET status = 'rejected', resolved_at = NOW(), resolved_by = ?, resolution_note = ?
         WHERE approval_id = ?`,
        [ctx.user.id, input.note ?? null, input.approvalId],
      );
