...
export async function calculateAIHealthScore(userId: number): Promise<AIHealthScore> {
  ...
  const overall = Math.round(
    securityScore.score * 0.40 +
    costScore.score * 0.35 +
...
async function calculateSecurityScore(userId: number): Promise<AIHealthScore["security"]> {
  try {
    ...
    const redTeamScore = redTeamHistory.length > 0
      ? Math.round(
          redTeamHistory.reduce((sum, r) => sum + ((r as Record<string,unknown>).securityScore as number || 0), 0) /
          redTeamHistory.length
        )
      : 100;

    const totalGatewayCalls = gatewayAudit.reduce((sum, r) => sum + r.allowedCount + r.blockedCount, 0);
    const blockedCalls = gatewayAudit.reduce((sum, r) => sum + r.blockedCount, 0);
    const promptInjectionBlockRate = totalGatewayCalls > 0
      ? Math.round((blockedCalls / totalGatewayCalls) * 100)
      : 100;
    ...
    const score = Math.round(
      severityScore * 0.4 +
      redTeamScore * 0.3 +
      Math.min(100, promptInjectionBlockRate) * 0.3
    );
...
async function calculateCostScore(userId: number): Promise<AIHealthScore["cost"]> {
  try {
    ...
    const currentSpend = tokenUsage.reduce(
      (sum, row) => sum + parseFloat((row as Record<string,unknown>).costUSD as string || "0"),
      0
    );
    const spendPercentage = budgetLimit > 0
    ...
    const forecastNextMonth = currentSpend > 0
      ? roundUSD(currentSpend)
      : 0;
...
async function calculateComplianceScore(userId: number): Promise<AIHealthScore["compliance"]> {
  try {
    ...
    if (soc2Evidence.length > 0) {
      ...
      const hasBlocked = soc2Evidence.some((r: Record<string,unknown>) => r.decision === "blocked");
    ...
    const complianceScore = Math.round(
      (soc2ControlsMet / soc2ControlsTotal) * 100
    );
  ...
  try {
    ...
    const spendPct = killSwitch
      ? (parseFloat(killSwitch.currentSpendUSD || "0") /
         parseFloat(killSwitch.budgetLimitUSD || "100")) * 100
      : 0;
    ...
    if (spendPct > 90) {
      risks.push({
        ...
        actionable: "Increase your budget limit or review which models are consuming the most tokens.",
      });
    } else if (spendPct > 75) {
      risks.push({
        category: "cost",
...
function generateRecommendations(
  ...
  if (security.score < 60) {
    recs.push("🔴 Your security posture needs immediate attention. Resolve critical findings first.");
  }
  if (cost.score < 60) {
  ...
  if (security.score >= 80 && cost.score >= 80 && compliance.score >= 70) {
    recs.push("🟢 Your AI operations are healthy! Schedule monthly red-team runs to maintain posture.");
  }

  if (cost.spendPercentage > 75) {
    recs.push("💡 Enable the kill switch to automatically halt LLM calls if you exceed 100% of budget.");
  }

  if (security.redTeamScore < 50) {
    recs.push("💡 Your prompt injection defenses are weak. Enable continuous red-teaming to find gaps.");
  }
