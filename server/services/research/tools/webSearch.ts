...
registerTool({
  ...
  handler: async (input): Promise<McpToolResult> => {
    ...
    if (!ENV.tavilyApiKey) {
      return {
        content: [{ type: "text", text: "Error: TAVILY_API_KEY not configured. Set it in .env to enable web search." }],
        isError: true,
      };
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.warn({ status: response.status, error: errorText }, "[Research] Tavily search failed");
        return {
          content: [{ type: "text", text: `Search failed (HTTP ${response.status}): ${errorText.slice(0, 500)}` }],
          isError: true,
        };
      }
...
function formatSearchResults(data: TavilyResponse, query: string, durationMs: number): string {
  ...
  data.results.forEach((r, i) => {
    ...
    if (r.raw_content) {
      lines.push(`   <details><summary>Full content</summary>${r.raw_content.slice(0, 2000)}</details>`);
    }
    lines.push("");
  });
