...
registerTool({
  ...
  handler: async (input): Promise<McpToolResult> => {
    ...
    if (!ENV.firecrawlApiKey) {
      return {
        content: [{ type: "text", text: "Error: FIRECRAWL_API_KEY not configured. Set it in .env to enable web scraping." }],
        isError: true,
      };
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          content: [{ type: "text", text: `Scrape failed (HTTP ${response.status}): ${errorText.slice(0, 500)}` }],
          isError: true,
        };
      }
...
function formatScrapeResult(
  ...
  if (data.metadata?.language) {
    lines.push(`**Language:** ${data.metadata.language} | **Status:** ${data.metadata.statusCode || "N/A"}`);
  }
  lines.push("");
