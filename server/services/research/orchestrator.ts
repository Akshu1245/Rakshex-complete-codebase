...
export async function executeResearch(input: ResearchJobInput): Promise<ResearchReport> {
  ...
  if (input.userId) {
    await researchDb.createResearchMemory({
      userId: input.userId,
      topic: input.topic,
...
async function executeParallelSearches(
  ...
  const results = new Map<string, Array<{ url: string; title: string; snippet: string; score: number }>>();
  ...
  const searchPromises = queries.map(async (query) => {
    try {
      ...
      if (result.isError) {
        logger.warn({ query, error: result.content[0]?.text }, "[Research] Search failed for query");
        return;
      }
...
function parseSearchResults(text: string): Array<{ url: string; title: string; snippet: string; score: number }> {
  ...
  for (const line of lines) {
    ...
    if (match) {
      if (currentItem.url) results.push(currentItem as { url: string; title: string; snippet: string; score: number });
...
async function executeParallelScrapes(
  urls: string[],
  depth: string
...
Generate a JSON response with these fields:
{
  ...
  try {
    ...
    const response = await invokeLLM({
      messages: [
        { role: "system", content: "You are a senior research analyst. Respond only with valid JSON." },
    ...
    const text = typeof choice.message.content === "string"
      ...
      : (choice.message.content as Array<{ type: string; text?: string }>).map(c => c.text || "").join("");
