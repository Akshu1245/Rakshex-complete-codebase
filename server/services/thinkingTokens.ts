...
function calculateThinkingAnalytics(usage: UsageRow[]): {
  ...
  const perModelMap = new Map<string, {
    totalCost: number;
    thinkingCost: number;
    thinkingTokens: number;
  }>();
  ...
  const perModel = Array.from(perModelMap.entries())
    .map(([model, data]) => ({
      ...
      thinkingPercentage: data.totalCost > 0
        ? Math.round((data.thinkingCost / data.totalCost) * 100)
        : 0,
    }))
  ...
  return {
    ...
    thinkingPercentage: totalCost > 0
      ? Math.round((totalThinkingCost / totalCost) * 100)
      : 0,
    totalThinkingTokens,
    perModel,
  };
