# DevPulse Terms of Service

> Last updated: May 17, 2026

---

## 1. ACCEPTANCE

By using DevPulse, you agree to these terms. If you disagree, do not use the service.

## 2. SERVICE DESCRIPTION

DevPulse provides:

- API security scanning and analysis
- LLM cost tracking and optimization
- Autonomous security monitoring (AgentGuard)
- Compliance report generation

## 3. ACCOUNT TERMS

- You must be 18 or older
- You are responsible for your account security
- One person per account (teams use workspace features)
- We may suspend accounts for abuse or illegal activity

## 4. ACCEPTABLE USE

### You MAY:

- Scan API collections you own or have permission to test
- Use findings to improve your own security
- Share workspace access with your team

### You MAY NOT:

- Scan systems you don't own without authorization
- Use DevPulse to attack, exploit, or harm others
- Resell DevPulse access without enterprise agreement
- Circumvent rate limits or access controls
- Upload malware or illegal content

## 5. DATA & PRIVACY

See [PRIVACY_POLICY.md](PRIVACY_POLICY.md). By using DevPulse, you agree to our privacy practices.

## 6. PAYMENT

| Plan       | Price  | Billing                 |
| ---------- | ------ | ----------------------- |
| Free       | $0     | —                       |
| Pro        | $29/mo | Monthly, cancel anytime |
| Enterprise | Custom | Annual or monthly       |

- Prices in USD
- Pro-rated refunds for annual plans within 30 days
- Failed payments: 7-day grace period, then downgrade to Free

## 7. SERVICE LEVEL

- **Target uptime:** 99.9%
- **Free tier:** Best effort, no SLA
- **Pro:** 99.5% uptime commitment
- **Enterprise:** 99.9% SLA with credits

## 8. INTELLECTUAL PROPERTY

- **You own:** Your API collections, scan results, custom policies
- **We own:** DevPulse software, algorithms, brand, documentation
- **Open source:** Scanner rules and SDKs are MIT licensed
- **Patents:** Thinking token attribution patent pending

## 9. LIMITATION OF LIABILITY

DevPulse is provided "as is". We are not liable for:

- Indirect, incidental, or consequential damages
- Loss of profits or data
- Security breaches in your own systems
- Actions taken based on scan results

Maximum liability: 12 months of fees paid.

## 10. TERMINATION

- You can delete your account anytime
- We may terminate for Terms violations
- Data deleted per Privacy Policy upon termination

## 11. GOVERNING LAW

These terms are governed by the laws of India. Disputes resolved in Bangalore courts.

## 12. CHANGES

We will notify of material changes 30 days in advance via email.

## 13. CONTACT

**Legal:** legal@devpulse.in
**Support:** support@devpulse.in
**Address:** DevPulse Technologies Pvt. Ltd., Bangalore, India

---

_Questions? Email legal@devpulse.in_
