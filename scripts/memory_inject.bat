...
powershell -Command "$q=$Env:QUERY; Get-ChildItem 'C:\Users\aksha\Downloads\DevPulse_Complete_Codebase' -Recurse -Include *.ts,*.tsx,*.js -Exclude node_modules,*test* 2>$null | Select-String -Pattern $q -SimpleMatch 2>$null | Select-Object -First 3 | ForEach-Object { Write-Host ('  [' + $_.Filename + ':' + $_.LineNumber + '] ' + $_.Line.Substring(0, [Math]::Min(120, $_.Line.Length))) }"
