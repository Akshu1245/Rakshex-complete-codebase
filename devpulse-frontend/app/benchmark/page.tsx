...
const PROVIDERS = [
  { id: "openai", label: "OpenAI", color: "#6366f1", models: ["gpt-4o", "gpt-4o-mini", "gpt-3.5-turbo"] },
  { id: "anthropic", label: "Anthropic", color: "#8b5cf6", models: ["claude-3-opus", "claude-3-sonnet", "claude-3-haiku"] },
  { id: "gemini", label: "Gemini", color: "#06b6d4", models: ["gemini-1.5-pro", "gemini-flash"] },
];

const BENCHMARK_RESULTS = [
  { model: "gpt-4o-mini", successRate: 98.2, avgLatency: 680, costPer1k: 0.15, p95: 1200, throughput: 45 },
  { model: "claude-3-haiku", successRate: 97.8, avgLatency: 720, costPer1k: 0.25, p95: 1350, throughput: 38 },
  { model: "gemini-flash", successRate: 96.5, avgLatency: 450, costPer1k: 0.075, p95: 890, throughput: 62 },
  { model: "gpt-4o", successRate: 99.1, avgLatency: 1800, costPer1k: 5.0, p95: 3200, throughput: 18 },
  { model: "claude-3-sonnet", successRate: 98.5, avgLatency: 1400, costPer1k: 3.0, p95: 2600, throughput: 22 },
];
...
export default function BenchmarkPage() {
  ...
  const [selectedModels, setSelectedModels] = useState(["gpt-4o-mini", "claude-3-haiku", "gemini-flash"]);
    ...
    <div className="text-white p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1">Compare models across speed, cost, quality, and reliability</p>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2">
              {PROVIDERS.map(provider => (
                <div key={provider.id}>
                  <p className="text-xs text-gray-500 mb-1" style={{ color: provider.color }}>{provider.label}</p>
                  <div className="flex flex-wrap gap-2">
                    {provider.models.map(model => (
                      <button key={model} onClick={() => toggleModel(model)}
                        ...
                        style={selectedModels.includes(model) ? { backgroundColor: provider.color + "30", borderColor: provider.color + "60", color: provider.color } : {}}>
                        {model}
                      </button>
                    ))}
                  </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {[{ label: "Request Count", value: "100" }, { label: "Prompt Type", value: "Mixed (summarization + Q&A + code)" }, { label: "Timeout (ms)", value: "5000" }].map(s => (
                <div key={s.label} className="flex items-center justify-between">
                  <span className="text-sm text-gray-400">{s.label}</span>
                  <span className="text-sm text-white bg-gray-700 px-3 py-1 rounded">{s.value}</span>
                </div>
              ))}
            </div>
            <button onClick={runBenchmark} disabled={running || selectedModels.length === 0}
              ...
              {running ? <><RefreshCw className="w-4 h-4 animate-spin" /> Running...</> : completed ? <><CheckCircle className="w-4 h-4" /> Re-run Benchmark</> : <><Play className="w-4 h-4" /> Run Benchmark</>}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-gray-800 rounded-xl border border-gray-700 p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={filteredResults}>
              ...
              <Bar dataKey="avgLatency" fill="#6366f1" radius={[4, 4, 0, 0]} name="Avg Latency (ms)" />
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={RADAR_DATA}>
              ...
              <Radar name="gpt-4o-mini" dataKey="gpt-4o-mini" stroke="#6366f1" fill="#6366f1" fillOpacity={0.15} />
              <Radar name="claude-3-haiku" dataKey="claude-3-haiku" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.15} />
              <Radar name="gemini-flash" dataKey="gemini-flash" stroke="#06b6d4" fill="#06b6d4" fillOpacity={0.15} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-700">
                {["Model", "Success Rate", "Avg Latency", "P95", "Cost/1K tokens", "Throughput (req/s)"].map(h => (
                  <th key={h} className="text-left px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filteredResults.sort((a, b) => b.successRate - a.successRate).map((r, i) => (
                <tr key={r.model} className="border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors">
