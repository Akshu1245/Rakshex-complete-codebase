...
import { Upload, Shield, Zap, DollarSign, AlertTriangle, CheckCircle, ChevronRight, Lock, FileJson, ArrowRight } from "lucide-react";
  ...
  const performClientScan = (collection: any): ScanResult => {
    ...
    const walkItems = (items: any[], path: string = "") => {
      items?.forEach((item, idx) => {
        const currentPath = path ? `${path} > ${item.name || `Item ${idx}`}` : (item.name || `Item ${idx}`);

        if (item.request) {
          const req = item.request;
          const url = typeof req.url === "string" ? req.url : req.url?.raw || req.url?.host?.join(".") || "unknown";
          ...
          if (/\{\{userId\}\}|\{userId\}|\/:userId|\/\d+/.test(url) && !headerNames.includes("authorization")) {
            findings.push({
              id: `find-${findings.length}`,
              severity: "Critical",
    ...
    const uniqueFindings = findings.filter((f, i, arr) => 
      arr.findIndex(t => t.title === f.title && t.endpoint === f.endpoint) === i
    );

    // Deduplicate credentials
    const uniqueCredentials = credentials.filter((c, i, arr) =>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-purple-500/20 border border-purple-400/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-sm font-medium text-purple-300">Zero setup · No signup · Instant results</span>
          </div>
          ...
          <p className="text-xl text-slate-300 max-w-2xl mx-auto">
            Drop your Postman collection. We will find exposed API keys, OWASP vulnerabilities, 
            and estimate your security risk — instantly, for free, no account required.
          </p>
        </div>
        ...
        {result && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className={`p-6 rounded-xl border ${criticalCount > 0 ? "bg-red-500/10 border-red-400/30" : "bg-green-500/10 border-green-400/30"}`}>
                <p className="text-sm text-slate-400 mb-1">Risk Score</p>
                <p className={`text-4xl font-bold ${criticalCount > 0 ? "text-red-400" : "text-green-400"}`}>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                ...
                <p className={`text-4xl font-bold ${result.owaspScore >= 80 ? "text-green-400" : result.owaspScore >= 50 ? "text-yellow-400" : "text-red-400"}`}>
                  {result.owaspScore}
                </p>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                ...
                <p className={`text-4xl font-bold ${result.pciScore >= 80 ? "text-green-400" : result.pciScore >= 50 ? "text-yellow-400" : "text-red-400"}`}>
                  {result.pciScore}
                </p>
                <p className="text-xs text-slate-500 mt-1">/100 compliance</p>
              </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-red-400">
                      🚨 {result.credentials.length} Exposed Credential{result.credentials.length > 1 ? "s" : ""} Found
                    </h3>
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm font-mono text-slate-400 mb-2">{finding.endpoint}</p>
                            <div className="bg-slate-900/50 rounded p-3">
                              <p className="text-sm text-slate-300">
                                <span className="text-green-400 font-medium">Fix:</span> {finding.remediation}
                              </p>
                            </div>
                          </div>
                        </div>
            ...
            {result.findings.length === 0 && result.credentials.length === 0 && (
              <div className="bg-green-500/10 border border-green-400/30 rounded-xl p-8 text-center">
                ...
                <p className="text-slate-300">No vulnerabilities or exposed credentials found in this collection.</p>
              </div>
            )}
            ...
            <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 border border-purple-400/30 rounded-xl p-8 text-center">
              ...
              <p className="text-slate-300 mb-6 max-w-lg mx-auto">
                Get real-time scans as you code, automatic PR checks, cost anomaly alerts, 
                and team dashboards — all inside VS Code.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a
                  href="https://devpulse.in/signup"
        ...
        {!result && (
          <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div className="p-6">
              ...
              <p className="text-sm text-slate-400">Detects BOLA, broken auth, injection, and more</p>
            </div>
            <div className="p-6">
              <DollarSign className="w-10 h-10 text-purple-400 mx-auto mb-3" />
              <h3 className="font-bold mb-2">LLM Cost Intelligence</h3>
              <p className="text-sm text-slate-400">Track token spend per endpoint and catch anomalies</p>
            </div>
            <div className="p-6">
              <Lock className="w-10 h-10 text-purple-400 mx-auto mb-3" />
              <h3 className="font-bold mb-2">Secret Detection</h3>
              <p className="text-sm text-slate-400">Finds API keys, tokens, and passwords in collections</p>
            </div>
          </div>
        )}
      </div>
