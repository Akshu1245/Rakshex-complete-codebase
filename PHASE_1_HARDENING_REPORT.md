...
| 8 | Strict CORS allowlist | ✓ done | `buildCorsAllowlist()` returns exactly `https://devpulse.in`, `https://www.devpulse.in`, `https://app.devpulse.in`, plus the `FRONTEND_URL` override. No wildcard, no origin-reflection. Trusts a single proxy hop in production so `X-Forwarded-For` cannot be spoofed past the rate limiter. |
