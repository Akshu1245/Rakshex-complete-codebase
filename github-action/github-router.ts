...
export const githubRouter = router({
  ...
  webhook: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Find the DevPulse user/org that owns this repo
      const org = await ctx.db.query.organizations.findFirst({
        where: eq(organizations.githubRepo, repository.full_name),
      });

      if (!org) {
        return { status: "ignored", reason: "Repository not linked to DevPulse" };
      }
  ...
  scan: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.openapiPath) {
        specFindings = await scanService.scanOpenAPISpec(
          `${repoPath}/${input.openapiPath}`
        );
      }
      ...
      const scanRecord = await ctx.db.insert(scans).values({
        userId,
        repository: input.repository,
      ...
      const complianceScore = {
        owasp: Math.max(0, 100 - (scoredFindings.filter((f) => f.severity === "Critical").length * 15 +
          scoredFindings.filter((f) => f.severity === "High").length * 8)),
        pci: Math.max(0, 100 - (scoredFindings.filter((f) => f.severity === "Critical").length * 20 +
  ...
  getPRScan: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const scan = await ctx.db.query.scans.findFirst({
        where: and(
          eq(scans.repository, input.repository),
          eq(scans.prNumber, input.prNumber)
        ),
