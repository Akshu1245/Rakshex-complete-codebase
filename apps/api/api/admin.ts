...
import { listWaitlistGrowthEntries } from "../services/waitlistGrowth";
...
export const adminRouter = router({
  ...
  listAllWaitlist: adminProcedure.query(async () => {
    const entries = await listWaitlistGrowthEntries();
    return { entries };
  }),
  ...
  changeUserPlan: adminProcedure
    .input(z.object({ userId: z.number(), plan: z.enum(["free", "pro", "enterprise"]) }))
    .mutation(async ({ input }) => {
