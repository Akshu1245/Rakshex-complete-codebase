...
function LoginForm() {
  ...
  // Check which OAuth providers are actually configured on the backend.
  // This prevents showing broken buttons when env vars aren't set.
  const { data: providers } = trpc.authProviders.list.useQuery(undefined, {
    staleTime: 5 * 60 * 1000, // 5 min — provider config rarely changes at runtime
  });
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex">
      <div className="w-full lg:w-[480px] flex flex-col items-center justify-center p-8">
        <div className="w-full max-w-[400px]">
          ...
          {/* Only render OAuth section when at least one provider is configured */}
          {hasOAuth && (
            <>
