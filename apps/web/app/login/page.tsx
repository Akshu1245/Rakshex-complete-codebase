...
function LoginForm() {
  ...
  const login = trpc.auth.login.useMutation({
    onSuccess: async (data) => {
      ...
      // auth.me is cached by the root AuthProvider. Before this fix it could
      // still contain the pre-login null value when /dashboard mounted,
      // causing AuthGuard to bounce a successfully authenticated user back
      // to /login. The login response has already delivered the HttpOnly
      // session cookie at this point, so invalidate and refetch the app
      // session before navigating to a protected page.
      await utils.auth.me.invalidate();
      const session = await utils.auth.me.fetch();
      if (!session) {
        setError("Sign-in succeeded, but the app session could not be established. Please try again.");
        return;
      }
      ...
      <div className="w-full lg:w-[480px] flex flex-col items-center justify-center p-8">
        <div className="w-full max-w-[400px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 mb-6">
            <div>
              <label htmlFor="email" className="block text-xs text-neutral-400 mb-1">Email</label>
              <input
                id="email"
                type="email"
                autoComplete="email"
            ...
            <div>
              <label htmlFor="password" className="block text-xs text-neutral-400 mb-1">Password</label>
              <PasswordField
                id="password"
                name="password"
                value={password}
          ...
          <p className="text-center text-neutral-500 text-sm">
            ...
            <Link href="/register" className="text-teal-400 hover:underline">Create account</Link>
          </p>
        </div>
      </div>
